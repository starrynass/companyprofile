...
<div class="container-fluid px-0">
    ...
    <div class="card border-0 shadow-sm rounded-4 bg-white overflow-hidden">
        <div class="card-header bg-transparent border-0 pt-4 px-4 pb-0 d-flex justify-content-between align-items-center">
            <h5 class="fw-bold text-dark mb-0"><i class="fa-solid fa-book-open text-indigo me-2" style="color: #6366f1;"></i>Daftar Galeri Publikasi</h5>
            <span class="badge bg-light text-secondary border px-3 py-2 rounded-pill fw-semibold">Total: {{ count($galeris) }} Foto</span>
        </div>
                ...
                <table class="table table-hover align-middle mb-0">
                    <thead class="text-secondary small text-uppercase bg-light rounded-3" style="font-size: 0.75rem; letter-spacing: 0.5px;">
                        <tr>
                            ...
                            <th class="py-3">Judul Foto</th>
                            <th class="py-3">Foto / Gambar</th>
                        ...
                        @forelse($galeris as $index => $item)
                            <tr class="transition-all" style="transition: background-color 0.2s ease;">
                                ...
                                <td>
                                    ...
                                    <small class="text-muted text-truncate d-inline-block" style="max-width: 300px;">{{ $item->foto }}</small>
                                </td>
                                <td class="text-end pe-3">
                                    <div class="d-inline-flex gap-1">
                                        ...
                                        <button type="button" class="btn btn-sm rounded-pill px-3 py-1.5 d-inline-flex align-items-center gap-1 transition-all" 
                                                ...
                                                data-bs-toggle="modal" data-bs-target="#editGaleriModal{{ $item->id }}"
                                        ...
                                        <!-- Tombol Hapus -->
                                        <form action="{{ route('admin.galeri-admin.destroy', $item->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Yakin ingin menghapus foto ini?');">
                                            @csrf
                                            @method('DELETE')
                            ...
                            <div class="modal fade" id="editGaleriModal{{ $item->id }}" tabindex="-1" aria-hidden="true">
                                <div class="modal-dialog modal-lg modal-dialog-centered">
                                    <div class="modal-content border-0 rounded-4 shadow-lg overflow-hidden">
                                        <form action="{{ route('admin.galeri-admin.update', $item->id) }}" method="POST">
                                            @csrf
                                            @method('PUT')
                                            <div class="modal-header border-0 pb-0 pt-4 px-4" style="background: linear-gradient(135deg, #f8fafc 0%, #f1f5f9 100%);">
                                                <div>
                                                    <span class="badge bg-indigo-subtle text-indigo mb-1" style="background-color: #e0e7ff; color: #4338ca; font-size: 0.75rem;">Edit Galeri</span>
                                                    <h5 class="fw-bold text-dark mb-0">Ubah Galeri: {{ $item->judul }}</h5>
                                                </div>
                                            ...
                                            <div class="modal-body p-4" style="background: #f8fafc;">
                                                <div class="bg-white p-4 rounded-4 shadow-sm border border-light">
                                                    <div class="row g-3">
                                                        <div class="col-md-8">
                                                            <label class="form-label small fw-semibold text-secondary">Judul Foto</label>
                                                        ...
                                                        <div class="col-12">
                                                            <label class="form-label small fw-semibold text-secondary">Foto (Nama File / URL Gambar)</label>
                                                            <input type="text" class="form-control rounded-3 py-2 bg-light border-0" name="foto" value="{{ $item->foto }}" placeholder="Contoh: foto-1.jpg" required style="font-size: 0.9rem;">
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                            ...
                            <tr>
                                <td colspan="5" class="text-center py-5 text-muted">
                                    <div class="py-4">
                                        ...
                                        <p class="mb-0 fw-semibold text-secondary">Belum ada data galeri yang tersedia.</p>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
...
<div class="modal fade" id="tambahGaleriModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-lg modal-dialog-centered">
        <div class="modal-content border-0 rounded-4 shadow-lg overflow-hidden">
            <form action="{{ route('admin.galeri-admin.store') }}" method="POST">
                @csrf
                <div class="modal-header border-0 pb-0 pt-4 px-4" style="background: linear-gradient(135deg, #f8fafc 0%, #f1f5f9 100%);">
                    <div>
                        ...
                        <h5 class="fw-bold text-dark mb-0">Tambah Galeri Baru</h5>
                    </div>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body p-4" style="background: #f8fafc;">
                    <div class="bg-white p-4 rounded-4 shadow-sm border border-light">
                        <div class="row g-3">
                            ...
                            <div class="col-12">
                                <label class="form-label small fw-semibold text-secondary">Foto (Nama File / URL Gambar)</label>
                                <input type="text" class="form-control rounded-3 py-2 bg-light border-0" name="foto" placeholder="Contoh: foto-1.jpg" required style="font-size: 0.9rem;">
                            </div>
                        </div>
                    </div>
                </div>
